...
async function runLoop(clients, round = 1) {
  ...
  const offset = ((round - 1) * BATCH_SIZE) % clients.length;
  const selectedClients = clients.slice(offset, offset + BATCH_SIZE);

  // sona geldiysek baştan tamamla
  if (selectedClients.length < BATCH_SIZE) {
    selectedClients.push(
      ...clients.slice(0, BATCH_SIZE - selectedClients.length)
    );
  }
  ...
  await Promise.all(
    selectedClients.map(async (client, idx) => {
      try {
        ...
        console.error(
          `Proxy #${((offset + idx) % clients.length) + 1} → Hata: ${
            err.message
          }`
        );
      }
  ...
  console.log(`⏱ Başarılı istek: ${success}/${BATCH_SIZE}`);
  console.log(`🚀 En uzun süre: ${max.toFixed(2)} ms`);
  console.log("-----------------------------------------------------\n");

  const delay = max < 1000 ? 1000 - max : 0;
  await wait(delay);
  return runLoop(clients, round + 1);
}
  ...
  const clients = await Promise.all(
    proxies.map(async (proxy, i) => {
      ...
      try {
        ...
        client.on("error", (e) =>
          console.error(`Client #${i + 1} H2 err:`, e.message)
        );
